...
class MapSetting:
    ...
    max_num_timesteps: int =  1 # max number of timesteps
    max_num_orders: int = 3         # max number of orders

    num_agents: int = 2  # fixed
    order_plus: bool = False
...
class OvercookedEnvironment(gym.Env):
    ...
    def reset(self):
        ...
        self.map_name = LVEL_TO_MAP[self.arglist.level]
    ...
    def step(self, action_dict, passed_time=1.):
        ...
        self.update_object_states_grill()
    ...
    def update_object_states(self):
        ...
        # update agents puting out fire(鍋に火が通り過ぎた際の処理)
        ...
        # update cooking or cooked objects(PotにObjectが入った時の処理)
    ...
    def update_object_states_grill(self):
        '''update the state for dynamic objects
        '''
      
        for pan in self.world.get_all_gridsquares('FryingPan'):
            if pan.holding is not None and (
                    pan.holding.is_cooking() or pan.holding.is_grill()) and 'Fire' in pan.holding.full_name:
                fire = [
                    x for x in pan.holding.contents if isinstance(x, Fire)][0]
                for agent in self.sim_agents:
                    if agent.action == (0, 0):
                        continue
                    action_x, action_y = self.world.inbounds(
                        tuple(np.asarray(agent.location) + np.asarray(agent.action)))
                    if action_x != pan.location[0] or action_y != pan.location[1]:
                        fire.remove_agent_putout_state(agent.name)

        pans = self.world.get_all_gridsquares('FryingPan')
        for pan in pans:
            if pan.holding is not None and (pan.holding.is_cooking() or pan.holding.is_grill()):
                self.world.remove(pan.holding)
                pre_update_name = pan.holding.full_name
                pan.holding.grill_update_state(self.current_time)
                after_update_name = pan.holding.full_name
                self.world.insert(pan.holding)

                # the food turns into fire
                if 'Burn' not in pre_update_name and 'Burn' in after_update_name:
                    fire = Fire()
                    self.world.remove(pan.holding)
                    pan.holding.contents.append(fire)
                    self.world.insert(pan.holding)
        ...
        for pan in self.world.get_all_gridsquares('FryingPan'):
            if pan.holding is not None and (
                ...
                if fire.is_finished:
                    self.world.remove(pan.holding)
                    pan.holding.contents.remove(fire)
                    self.world.insert(pan.holding)
    ...
    def get_current_state(self):
        ...
        # objects
        ALL_ENTITIES = FRESH_FOOD + CHOPPING_FOOD + CHOPPED_FOOD + COOKING_FOOD +\
                       COOKED_FOOD + CHARRED_FOOD + GRILL_FOOD+ BURN_FOOD + ['Plate',
                                                                              'Fire',
                                                                              'FireExtinguisher']
