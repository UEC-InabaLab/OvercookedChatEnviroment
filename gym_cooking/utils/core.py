...
# -----------------------------------------------------------
# Additional Code
class FryingPan(GridSquare):
    def __init__(self, location):
        GridSquare.__init__(self, "FryingPan", location)
        self.rep = Rep.FRYING_PAN
        self.collidable = True
...
ValidFoodNames = ('ChoppedLettuce', 'ChoppedOnion', 'ChoppedTomato','ChoppedBellpepper',
                  ...
                  'ChoppedBellpepper-ChoppedLettuce','ChoppedBellpepper-ChoppedOnion','ChoppedBellpepper-ChoppedTomato',
                  'ChoppedBellpepper-ChoppedLettuce-ChoppedOnion','ChoppedBellpepper-ChoppedLettuce-ChoppedTomato','ChoppedBellpepper-ChoppedOnion-ChoppedTomato'
                  )
...
class Object:
    ...
    def grill_update_state(self, current_tiem):
        for c in self.contents:
            c.grill_update_state(current_tiem)
    ...
    def is_grill(self):
        for c in self.contents:
            if not ((isinstance(c, Food) and c.is_grill()) or isinstance(c, Fire)):
                return False
        return True
...
def mergeable(obj1, obj2):
    ...
    if (obj1.is_cooked() or obj1.is_grill()) and len(obj2.contents) == 1 and Plate() in obj2.contents:
        return True
    if (obj2.is_cooked() or obj1.is_grill()) and len(obj1.contents) == 1 and Plate() in obj1.contents:
        return True
...
class FoodState:
    ...
    GRILL = getattr(RECIPY_UTIL, 'Grill')
    BURN = getattr(RECIPY_UTIL, 'Burn')
    DELIVERABLE = [COOKED, GRILL]
...
class FoodSequence:
    ...
    FRESH_CHOPPING_CHOPPED_COOKING_COOKED = FRESH_CHOPPING_CHOPPED + [FoodState.COOKING, FoodState.COOKED, FoodState.GRILL]
    #FRESH_CHOPPING_CHOPPED_COOKING_COOKED_CHARRED = FRESH_CHOPPING_CHOPPED_COOKING_COOKED + [FoodState.CHARRED, FoodState.BURN]
    FRESH_CHOPPING_CHOPPED_COOKING_COOKED_CHARRED = FRESH_CHOPPING_CHOPPED + [FoodState.COOKING, FoodState.COOKED,\
                                                                       FoodState.CHARRED, FoodState.GRILL, FoodState.BURN]
...
class Food:
    ...
    def grill_update_state(self, current_time):
        if isinstance(self.state, FoodState.COOKING) or isinstance(self.state, FoodState.GRILL):
            self.state.update_by_current_time(current_time=current_time)
        else:
            self.state.update_one_step(passed=1)
        if self.state.is_finished:
            if isinstance(self.state, FoodState.COOKING):
                self.state_index += 3
            else:
                self.state_index += 1
            assert 0 <= self.state_index and self.state_index < len(self.state_seq), "State index is out of bounds for its state sequence"
    ...
    def is_grill(self):
        return isinstance(self.state, FoodState.GRILL) or isinstance(self.state, FoodState.BURN)
...
##
class BellpepperTile(Tile):
    def __init__(self, location):
        Tile.__init__(self, location, 'FreshBellpepperTile', Bellpepper)
        self.rep = Rep.BELLPEPPER_TILE


# -----------------------------------------------------------
# NAMES
# -----------------------------------------------------------
GRIDSQUARES = ["Floor", "Counter", "Cutboard", "Delivery", "Bin", "Pot", "FreshTomatoTile", "FreshOnionTile", "FreshLettuceTile","PlateTile", "FryingPan", "FreshBellpepperTile"]
PUTTABLE_GRIDSQUARES = ['Counter', 'Cutboard']
FOOD_TILE = ['FreshTomatoTile', 'FreshLettuceTile', 'FreshOnionTile', 'FreshBellpepperTile']
FRESH_FOOD = ['FreshTomato', 'FreshLettuce', 'FreshOnion','FreshBellpepper']
CHOPPING_FOOD = ['ChoppingTomato', 'ChoppingOnion', 'ChoppingLettuce','ChoppingBellpepper']
CHOPPED_FOOD = ['ChoppedTomato', 'ChoppedOnion', 'ChoppedLettuce','ChoppedBellpepper']
COOKING_FOOD = ['CookingTomato', 'CookingOnion', 'CookingLettuce','CookingBellpepper']
COOKED_FOOD = ['CookedTomato', 'CookedOnion', 'CookedLettuce','CookedBellpepper']
CHARRED_FOOD = ['CharredTomato', 'CharredOnion', 'CharredLettuce','CharredBellpepper']
GRILL_FOOD = ['GrillTomato', 'GrillOnion', 'GrillLettuce','GrillBellpepper']
BURN_FOOD = ['BurnTomato', 'BurnOnion', 'BurnLettuce','BurnBellpepper']
...
def add_something(obj_list, x):
    ...
    return ret
ASSEMBLE_CHOPPED_FOOD = ['ChoppedTomato', 'ChoppedOnion', 'ChoppedLettuce', 'ChoppedBellpepper',\
                         'ChoppedLettuce-ChoppedOnion','ChoppedLettuce-ChoppedTomato', 'ChoppedOnion-ChoppedTomato', 'ChoppedLettuce-ChoppedOnion-ChoppedTomato',\
                         'ChoppedBellpepper-ChoppedLettuce','ChoppedBellpepper-ChoppedOnion','ChoppedBellpepper-ChoppedTomato',\
                         'ChoppedBellpepper-ChoppedLettuce-ChoppedOnion', 'ChoppedBellpepper-ChoppedOnion-ChoppedTomato','ChoppedBellpepper-ChoppedLettuce-ChoppedTomato']
...
ASSEMBLE_GRILL_FOOD = [f.replace('Chopped', 'Grill') for f in ASSEMBLE_CHOPPED_FOOD]
ASSEMBLE_GRILL_PLATE_FOOD = add_something(ASSEMBLE_GRILL_FOOD, 'Plate')
ASSEMBLE_BURN_FOOD = [f.replace('Chopped', 'Burn') for f in ASSEMBLE_CHOPPED_FOOD]
ASSEMBLE_BURN_PLATE_FOOD = add_something(ASSEMBLE_BURN_FOOD, 'Plate')
...
RepToClass = {
    ...
    Rep.BELLPEPPER: globals()['Bellpepper'],
    ...
    Rep.BELLPEPPER_TILE : globals()['BellpepperTile'],
    Rep.FIRE_EXTINGUISHER: globals()['FireExtinguisher'],
    Rep.FRYING_PAN : globals()['FryingPan']
}    