...
class World:
    ...
    def __init__(self, arglist):
        ...
        self.map_name = LVEL_TO_MAP[arglist.level] # mapの名前を取得
    ...
    def get_lower_bound_between_helper(self, subtask, agent_locs, A_loc, B_loc):
        ...
        for A_na, B_na in product(A_possible_na, B_possible_na):
            if len(agent_locs) == 1:
                ...
                bound_between_agents = float(abs(A_loc[0] - B_loc[0]) + abs(A_loc[1] - B_loc[1]))
    ...
    def remove(self, obj):
        with self.lock:
            ...
            index = None
            #print(f"Objects for {obj.name}")
            for i in range(num_objs):
                if self.objects[obj.name][i].location == obj.location:
                    index = i
                    break  
            #print(f"After for {obj.name}")
            assert index is not None, f"Could not find {obj.name} at location {obj.location}! Current locations: {[o.location for o in self.objects[obj.name]]}"

            # index が有効であれば pop する
            if index is not None and index < len(self.objects[obj.name]):
                self.objects[obj.name].pop(index)
            else:
                print(f"Warning: Invalid index {index} for {obj.name} at location {obj.location}")
            
            assert len(self.objects[obj.name]) < num_objs, f"Nothing from {obj.name} was removed from world.objects"
