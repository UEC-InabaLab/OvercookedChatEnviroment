...
from gym_cooking.utils.core import GRIDSQUARES, PUTTABLE_GRIDSQUARES, FRESH_FOOD, CHOPPED_FOOD, CHOPPING_FOOD, \
    ...
    ASSEMBLE_CHARRED_FOOD, ASSEMBLE_CHARRED_PLATE_FOOD, ASSEMBLE_GRILL_FOOD, ASSEMBLE_GRILL_PLATE_FOOD, ASSEMBLE_BURN_FOOD, ASSEMBLE_BURN_PLATE_FOOD
...
def get_all_events(recipes):
    ...
    put = [f'Put_{f}_on_{gs}' for f, gs in
           itertools.product(
               ...
               + ASSEMBLE_CHARRED_PLATE_FOOD + ASSEMBLE_GRILL_PLATE_FOOD + ASSEMBLE_BURN_PLATE_FOOD +['FireExtinguisher', 'Plate'], PUTTABLE_GRIDSQUARES)] \
    ...
    pickup += [f'Pickup_{f}_from_{gs}' for f, gs in
              itertools.product(
                  FRESH_FOOD + ASSEMBLE_CHOPPED_FOOD + ASSEMBLE_CHOPPED_PLATE_FOOD + ASSEMBLE_COOKED_PLATE_FOOD
                  + ASSEMBLE_CHARRED_PLATE_FOOD + ASSEMBLE_GRILL_PLATE_FOOD + ASSEMBLE_BURN_PLATE_FOOD + ['FireExtinguisher', 'Plate'], PUTTABLE_GRIDSQUARES)] \
             + [f'Pickup_{f}_from_{gs}' for f, gs in itertools.product(ASSEMBLE_COOKED_FOOD + ASSEMBLE_CHARRED_FOOD + ASSEMBLE_GRILL_FOOD + ASSEMBLE_BURN_FOOD, ["FryingPan"])]
    
    deliver = [f'Deliver_{f}' for f in  ASSEMBLE_COOKED_PLATE_FOOD + ASSEMBLE_GRILL_PLATE_FOOD]

    drop = [f'Drop_{f}' for f in FRESH_FOOD + ASSEMBLE_CHOPPED_FOOD + ASSEMBLE_COOKED_FOOD + ASSEMBLE_CHARRED_FOOD + ASSEMBLE_GRILL_FOOD + ASSEMBLE_BURN_FOOD]

    putout = ['Putout_Fire']

    cut = [f"Cut_{f}" for f in CHOPPED_FOOD+CHOPPING_FOOD]


    return no_op + move + chop + cook + assemble + put + pickup + deliver + drop + putout + cut
